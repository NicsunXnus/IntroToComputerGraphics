...
void DrawDisc( const discType *d )
{
    ...
    if ( firstTime )
    {
        ...
        //****************************
        double angleIncrement = 2 * PI / NUM_OF_SIDES;
        for (int i = 0; i <= NUM_OF_SIDES; i++) {
            unitDiscVertex[i][0] = cos(i * angleIncrement);
            unitDiscVertex[i][1] = sin(i * angleIncrement);
        }
...
void MyMouse( int btn, int state, int x, int y )
{
    if ( btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN )
    {
        ...
        else
        {
            ...
            disc[numDiscs].speed[0] = (rand() % (int)(MAX_X_SPEED - MIN_X_SPEED + 1) + MIN_X_SPEED) * (rand() % 2 == 0 ? 1 : -1);
            disc[numDiscs].speed[1] = (rand() % (int)(MAX_Y_SPEED - MIN_Y_SPEED + 1) + MIN_Y_SPEED) * (rand() % 2 == 0 ? 1 : -1);
...
int main( int argc, char** argv )
{
    ...
    glutTimerFunc(1000 / DESIRED_FPS, MyTimer, 0);  //*** MODIFY THIS ***
