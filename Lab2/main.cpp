//============================================================
// STUDENT NAME: Nicholas Sun Jun Yang
// NUS User ID.: E0543645
// COMMENTS TO GRADER:
// How do I set up the gluLookAt in MyDisplay instead of using transformation matrixes?
// ============================================================
...
void DrawOneCar( float bodyColor[3] )
{
    ...
    //Bottom of car
    glPushMatrix();
        glScalef(CAR_LENGTH, CAR_WIDTH, CAR_HEIGHT/2); //x is length, y is width,z is height
        glTranslatef(0, 0.0, 0.5); //move bottom to z=0 plane
        glutSolidCube(1.0);
        //Top of car
        glPushMatrix();
            glScalef(0.5, 1, 2); //x is length, y is width,z is height
            glTranslatef(0, 0.0, 0.5); //move bottom to z=0 plane
            glutSolidCube(1.0); //origin at 0,0,0
        glPopMatrix();
        ...
        //Right wheel
        glPushMatrix();
            glTranslatef(-CAR_LENGTH / 2 + CAR_HEIGHT / 4, CAR_WIDTH / 2, CAR_HEIGHT / 4);
            glRotatef(90, 1, 0, 0);
            glutSolidTorus(CAR_HEIGHT / 8, CAR_HEIGHT / 4, 10, 20);
        glPopMatrix();
        //Left Wheel
        glPushMatrix();
            glTranslatef(-CAR_LENGTH / 2 + CAR_HEIGHT / 4, -CAR_WIDTH / 2, CAR_HEIGHT / 4);
...
void DrawAllCars( void )
{
    for ( int i = 0; i < NUM_CARS; i++ )
    {
        ...
        glPushMatrix();
            ...
            glTranslatef(PLANET_RADIUS * cos(car[i].angularPos *  PI/180), 0, PLANET_RADIUS * sin(car[i].angularPos * PI/180));
            glRotatef(90 - car[i].angularPos,0,1,0);
...
void MyDisplay( void )
{
    ...
    // far planes.
    //***********************************************************************
    double nearPlane = eyeDistance - CLIP_PLANE_DIST;
    double farPlane = eyeDistance + CLIP_PLANE_DIST;
    double aspectRatio = (double)winWidth / winHeight;
    gluPerspective( VERT_FOV, aspectRatio, nearPlane, farPlane );
    ...
    //***********************************************************************
    glTranslatef(0, 0, -eyeDistance); //Translate backwards first
    glRotatef(eyeLatitude, 1, 0, 0); //rotate vertically about x-axis
    glRotatef(eyeLongitude, 0, 1, 0); //rotate horizontally about y-axis
