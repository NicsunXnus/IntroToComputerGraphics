...
// Compute for shadow if hasShadow is true.
    ...
    for (const auto& ptLight : scene.ptLights)
    {
        ...
        if (hasShadow) {
            //loop breaks if there is occlusion
            for (const auto& surface : scene.surfaces) {
                Ray shadowRay(nearestHitRec.p, L);
                if (surface->shadowHit(shadowRay, DEFAULT_TMIN, newTmax)) {
                    isShadowHit = true;
                    break;
                }
            }
        ...
        //add phong lighting if not occluded
        if (!isShadowHit) {
...
// Add to result the reflection of the scene.
    ...
    Ray spawnReflectRay(nearestHitRec.p, reflectRayDir);
    return  result + nearestHitRec.material.k_rg * TraceRay(spawnReflectRay, scene, reflectLevels - 1, hasShadow);
}
