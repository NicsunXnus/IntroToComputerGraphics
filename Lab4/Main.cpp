...
void DefineScene2( Scene &scene, int imageWidth, int imageHeight )
{
    ...
    scene.amLight.I_a = Color(1.0f, 1.0f, 1.0f) * 0.50f;

    // Define materials.

    // strong yellow.
    Material strongYellow = Material();
    strongYellow.k_d = Color(1.0f, 1.0f, 0.2f);
    strongYellow.k_a = strongYellow.k_d;
    strongYellow.k_r = Color(0.8f, 0.8f, 0.8f) / 1.5f;
    strongYellow.k_rg = Color(0.8f, 0.8f, 0.8f) / 3.0f;
    strongYellow.n = 128.0f;
    ...
    black.k_d = Color(0.2f, 0.2f, 0.2f);
    black.k_a = black.k_d;
    black.k_r = Color(0.6f, 0.6f, 0.6f);
    black.k_rg = Color(0.8f, 0.8f, 0.8f) / 3.0f;
    black.n = 30.0f;

    // Insert into scene materials vector.
    scene.materials = { strongYellow, lightGreen, lightBlue, yellow, gray, white, black };
    ...
    auto rightVertPlane = new Plane(0.0, 0.0, 1.0, 0.0, scene.materials[6]); // Right vertical plane.
    // Punk SnowMan Body
    auto bigSphere = new Sphere(Vector3d(40.0, 20.0, 42.0), 50.0, scene.materials[5]); // Big sphere.
    auto medSphere = new Sphere(Vector3d(40.0, 80.0, 42.0), 35.0, scene.materials[5]); // Big sphere.
    auto smallSphere = new Sphere(Vector3d(40.0, 120.0, 42.0), 20.0, scene.materials[5]); // Small sphere.
    // Punk SnowMan Eyes
    auto leftEye = new Sphere(Vector3d(60.0, 120.0, 42.0 - 20.0 / 3), 5.0, scene.materials[6]);
    auto rightEye = new Sphere(Vector3d(60.0, 120.0, 42.0 + 20.0 / 3), 5.0, scene.materials[6]);
    // Punk SnowMan Hair
    //Equation of 2D circle : x^2 + y^2 = r^2
    // Hair 1 xy vertices
    double hairx11 = 28.0;
    double hairy11 = sqrt(20 * 20 - 12 * 12) + 120.0;
    double hairx21 = 36.0;
    double hairy21 = sqrt(20 * 20 - 4 * 4) + 120.0;
    double hairx31 = 32.0;
    double hairy31 = 40.0 + 120.0;
    // Hair 2 xy vertices
    double hairx12 = 36.0;
    double hairy12 = sqrt(20 * 20 - 4 * 4) + 120.0;
    double hairx22 = 44.0;
    double hairy22 = sqrt(20 * 20 - 4 * 4) + 120.0;
    double hairx32 = 40.0;
    double hairy32 = 40.0 + 120.0;
    // Hair 3 xy vertices
    double hairx13 = 44.0;
    double hairy13 = sqrt(20 * 20 - 4 * 4) + 120.0;
    double hairx23 = 52.0;
    double hairy23 = sqrt(20 * 20 - 12 * 12) + 120.0;
    double hairx33 = 48.0;
    double hairy33 = 40.0 + 120.0;

    auto hair1 = new Triangle(Vector3d(hairx11, hairy11, 42.0),
        Vector3d(hairx21, hairy21, 42.0),
        Vector3d(hairx31, hairy31, 42.0),
        scene.materials[0]);
    auto hair2 = new Triangle(Vector3d(hairx12, hairy12, 42.0),
        Vector3d(hairx22, hairy22, 42.0),
        Vector3d(hairx32, hairy32, 42.0),
        scene.materials[0]);
    auto hair3 = new Triangle(Vector3d(hairx13, hairy13, 42.0),
        Vector3d(hairx23, hairy23, 42.0),
        Vector3d(hairx33, hairy33, 42.0),
        scene.materials[0]);

    scene.surfaces = { horzPlane, leftVertPlane, rightVertPlane,
                       bigSphere,  medSphere, smallSphere,
                        leftEye, rightEye,
                        hair1, hair2, hair3
                       };
    ...
    scene.camera = Camera(Vector3d(350.0, 200.0, 200.0),  // eye
        Vector3d(40.0, 80.0, 42.0),  // lookAt
        Vector3d(0.0, 1.0, 0.0),  //upVector
        (-1.0 * imageWidth) / imageHeight,  // left
        (1.0 * imageWidth) / imageHeight,  // right
        -1.0, 1.0, 3.0,  // bottom, top, near
