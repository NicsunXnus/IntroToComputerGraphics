...
const char cloudTexFile[] = "images/bluecloud.jpg";
const char halloweenTexFile[] = "images/halloween.jpg";
const char skullTexFile[] = "images/skull.jpg";
...
void DrawCube( void ); //My addition
void DrawTable( void );
...
void MakeReflectionImage( void )
{
    ...
    // Set up view volume for imaginary vp
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glFrustum(TABLETOP_Y1 - eyePos[1], TABLETOP_Y2 - eyePos[1],
              TABLETOP_X1 - eyePos[0], TABLETOP_X2 - eyePos[0],
              eyePos[2] - TABLETOP_Z, eyePos[2] - TABLETOP_Z + SCENE_RADIUS);
    
    // STEP 3: Set up the imaginary viewpoint.
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(eyePos[0], eyePos[1], 2 * TABLETOP_Z - eyePos[2], eyePos[0], eyePos[1], 1,
              1.0, 0.0, 0.0);
    // STEP 4: Set up the light source positions in the world space.
    glEnable(GL_LIGHTING);
    glLightfv(GL_LIGHT0, GL_POSITION, light0Position);
    glLightfv(GL_LIGHT1, GL_POSITION, light1Position);
    // STEP 5: Draw the scene (may not need to draw all objects). Draw the things to be reflected.
    DrawRoom();
    DrawTeapot();
    DrawSphere();
    DrawPyramid(); // my addition
    DrawCube(); // my addition
...
// This texture is for the bluecloud texture map
    glGenTextures(1, &cloudTexObj);
    glBindTexture(GL_TEXTURE_2D, cloudTexObj);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                    GL_LINEAR);  // GL_NEAREST,GL_LINEAR
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
        GL_LINEAR_MIPMAP_LINEAR);  // GL_NEAREST,GL_LINEAR_MIPMAP_LINEAR

    if (ReadImageFile(cloudTexFile, &imageData, &imageWidth, &imageHeight,
                      &numComponents) == 0)
        exit(1);
    if (numComponents != 3) {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
    ...
    DeallocateImageData(&imageData);
// This texture is for the halloween texture map
    glGenTextures(1, &halloweenTexObj);
    glBindTexture(GL_TEXTURE_2D, halloweenTexObj);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                    GL_LINEAR);  // GL_NEAREST,GL_LINEAR
    ...
    if (ReadImageFile(halloweenTexFile, &imageData, &imageWidth, &imageHeight,
                      &numComponents) == 0)
        exit(1);
    if (numComponents != 3) {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
    ...
    DeallocateImageData(&imageData);
// This texture is for the skull texture map
    glGenTextures(1, &skullTexObj);
    glBindTexture(GL_TEXTURE_2D, skullTexObj);
    ...
    if (ReadImageFile(skullTexFile, &imageData, &imageWidth, &imageHeight,
                      &numComponents) == 0)
        exit(1);
    if (numComponents != 3) {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
void DrawCube(void) {
    ...
    glBindTexture(GL_TEXTURE_2D, cloudTexObj);
    glPushMatrix();
        glTranslated(0.6 , -0.5, 0.3 + TABLETOP_Z);
        // FRONT
        glBegin(GL_POLYGON);
        glVertex3f(-0.3, -0.3, -0.3);glTexCoord2f(0.0,0.0);
        glVertex3f(-0.3, 0.3, -0.3);glTexCoord2f(1.0,0.0);
        glVertex3f(0.3, 0.3, -0.3);glTexCoord2f(1.0,1.0);
        glVertex3f(0.3, -0.3, -0.3);glTexCoord2f(0.0,1.0);
        glEnd();
        // BACK
        glBegin(GL_POLYGON);
        glVertex3f(0.3, -0.3, 0.3); glTexCoord2f(0.0, 0.0);
...
void DrawPyramid() {
    /*
    * Coordinates of base and top of pyramid
     (-0.5,0.5,0)|----------| (0.5,0.5,0.0)
                 |          |
                 |  (0,0,1) | 
                 |          |
    (-0.5,-0.5,0)|__________| (0.5,-0.5,0)
    Texture mapping: Bot left to (0.0,0.0), Top left to (0.0,1.0), Top right to (1.0,1.0), Bot right to (1.0,0.0), Middle to (0.5,0.5)
    */
    GLfloat Av[] = {-0.5f, -0.5f, 0.0f};
    GLfloat Bv[] = { -0.5f, -0.5f, 0.0f };
    GLfloat Cv[] = { -0.5f, 0.5f, 0.0f };
    GLfloat Dv[] = { -0.5f, 0.5f, 0.0f };
    GLfloat Ev[] = { 0.5f, 0.5f, 0.0f };
    GLfloat Fv[] = { 0.5f, 0.5f, 0.0f };
    GLfloat Gv[] = { 0.5f, -0.5f, 0.0f };
    GLfloat Hv[] = { 0.5f, -0.5f, 0.0f };
    GLfloat Iv[] = { 0.0f, 0.0f, 1.0f };
    GLfloat Jv[] = { 0.0f, 0.0f, 1.0f };
    GLfloat Kv[] = { 0.0f, 0.0f, 1.0f };
    GLfloat Lv[] = { 0.0f, 0.0f, 1.0f };
    ...
    //Reference for drawing the pyramid. Shows order of vertices being used.
    //GLubyte sideIndices[12] = {0,  2,  8,   3, 4,  9,
    //                          5, 6, 10,   1, 7, 11};

    GLfloat matAmbient[] = {1.0, 1.0, 1.0, 1.0};
    GLfloat matDiffuse[] = {1.0, 1.0, 1.0, 1.0};
    GLfloat matSpecular[] = {0.5, 0.5, 0.5, 0.5};
    GLfloat matShininess[] = {32.0};
    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, matAmbient);
    glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, matDiffuse);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, matSpecular);
    glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, matShininess);
    ...
    glPushMatrix();
        glTranslated(-0.45, 0.5, 1.0 + TABLETOP_Z);
        glBegin(GL_TRIANGLES);
        glTexCoord2fv(Atc);  glVertex3fv(Av);
        glTexCoord2fv(Ctc);  glVertex3fv(Cv);
        glTexCoord2fv(Itc);  glVertex3fv(Iv);
        glEnd();
        glBegin(GL_TRIANGLES);
        glTexCoord2fv(Dtc);  glVertex3fv(Dv);
        glTexCoord2fv(Etc);  glVertex3fv(Ev);
        glTexCoord2fv(Jtc);  glVertex3fv(Jv);
        glEnd();
        glBegin(GL_TRIANGLES);
        glTexCoord2fv(Ftc);  glVertex3fv(Fv);
        glTexCoord2fv(Gtc);  glVertex3fv(Gv);
        glTexCoord2fv(Ktc);  glVertex3fv(Kv);
        glEnd();
        glBegin(GL_TRIANGLES);
        glTexCoord2fv(Btc);  glVertex3fv(Bv);
        glTexCoord2fv(Htc);  glVertex3fv(Hv);
        glTexCoord2fv(Ltc);  glVertex3fv(Lv);
        glEnd();
    glPopMatrix();
    glEnable(GL_CULL_FACE);

    //Draw Base
    GLfloat baseAv[] = { -0.5f, -0.5f, 0.0f };
    GLfloat baseBv[] = { -0.5f, 0.5f, 0.0f };
    GLfloat baseCv[] = { 0.5f, 0.5f, 0.0f };
    GLfloat baseDv[] = { 0.5f, -0.5f, 0.0f };

    GLfloat baseAtc[] = {0.0,0.0};
    GLfloat baseBtc[] = { 0.0,1.0 };
    GLfloat baseCtc[] = { 1.0,1.0 };
    GLfloat baseDtc[] = { 1.0,0.0 };

    glBindTexture(GL_TEXTURE_2D, skullTexObj);
    glPushMatrix();
        glTranslated(-0.45, 0.5, 1.0 + TABLETOP_Z);
        glBegin(GL_QUADS);
        glTexCoord2fv(baseAtc);  glVertex3fv(baseAv);
        glTexCoord2fv(baseBtc);  glVertex3fv(baseBv);
        glTexCoord2fv(baseCtc);  glVertex3fv(baseCv);
        glTexCoord2fv(baseDtc);  glVertex3fv(baseDv);
        glEnd();
    glPopMatrix();
}
...
// Tabletop.
    ...
    // Bind the reflection texture object to the GL_TEXTURE_2D target.
    glBindTexture(GL_TEXTURE_2D, reflectionTexObj);

    // Draw the tabletop rectangle using SubdivideAndDrawQuad().
    glNormal3f(0.0, 0.0, 1.0); // Normal vector.
